...
const O337X_SITE = process.env.O337X_SITE || "https://1337x.unblockit.onl/search/{term}/1/";
...
async function search(search, site = O337X_SITE) {
  try {
    ...
    var searchResults = await page.evaluate(async () => {
      ...
      if (!searchResults) {
        return { error: true, errorMessage: "Tidak ditemukan" };
      }
    ...
    console.log(err);
    return { error: true, errorMessage: "Terjadi kesalahan saat pemrosesan" };
  }
}
