...
class Torrent {
  ...
  statusLoader = torrent => {
    return {
      status: torrent.done ? "Sudah terunduh" : torrent.name ? "Sedang mengunduh" : "Menggali metadata",
      magnetURI: torrent.magnetURI,
      name: torrent.name,
      speed: `${prettyBytes(torrent.downloadSpeed)}/dtk`,
	...
	//  downloadLink: `https://drive.helmajs.my.id/:1/${torrent.name}`,
      done: torrent.done
    };
  };
